...
Если у вас что-то не получилось, оформите Issue [Шаблон для оформления есть по ссылке](../report-requirements.md).
...
Если тесты отказываются запускаться или происходят непонятные проблемы с jacoco, удалите из build.gradle следующие строки:
...
Мы настроим CI на базе GitHub Actions, уже встроенный в GitHub-системы.
...
Задача – подключить к вашему репозиторию GitHub Actions, следуя инструкции выше.
...
```
В таком случае Jacoco-отчёт будет сгенерирован на каждый пуш в master/main ветку. Либо на каждый пуш в ветку, на которую открыт PR в master/main. И загружен на Github в виде артефакта для скачивания.
